...
resource "azurerm_public_ip" "pip" {
  ...
  name                = "${var.server_prefix}-${var.servers[count.index].server_name}-pip-${count.index}"  # Ensure unique name with prefix and index
  resource_group_name = data.azurerm_resource_group.hubrg01.name
  location            = data.azurerm_resource_group.hubrg01.location
...
resource "azurerm_network_interface" "nic" {
  count               = length(var.servers)
  name                = "${var.server_prefix}-${var.servers[count.index].server_name}-nic-${count.index}"  # Ensure unique name with prefix and index
  resource_group_name = data.azurerm_resource_group.hubrg01.name
  location            = data.azurerm_resource_group.hubrg01.location

  ip_configuration {
    name                          = "internal-${count.index}"  # Ensure unique name
...
resource "azurerm_windows_virtual_machine" "vm" {
  count               = length(var.servers)
  name                = "${var.server_prefix}-${var.servers[count.index].server_name}-${count.index}"  # Ensure unique name with prefix and index
  location            = data.azurerm_resource_group.hubrg01.location
  resource_group_name = data.azurerm_resource_group.hubrg01.name
